...
/**
 * The Main class serves as the entry point for the file system simulation.
 * It demonstrates the creation of drives, home directories, and files within the file system.
 * It also shows how to append directories and files to the file system and calculate the total size of each drive.
 */
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Directory repoSrc = new Directory(repo, "src", LocalDateTime.now());
        Directory repoTest = new Directory(repo, "test", LocalDateTime.now());
        Directory testSrc = new Directory(repo, "src", LocalDateTime.now());

        // Create files within the home directories
        File readme_md = new File(repo, "readme.md", 100, LocalDateTime.now());
        File A_java = new File(repoSrc, "A.java", 120, LocalDateTime.now());
        File B_java = new File(repoSrc, "B.java", 180, LocalDateTime.now());
        File A_javaTest = new File(testSrc, "ATest.java", 90, LocalDateTime.now());
        File B_javaTest = new File(testSrc, "BTest.java", 200, LocalDateTime.now());
